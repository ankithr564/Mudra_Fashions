...
export const RoleSwitcher: React.FC = () => {
  ...
  const roles: { key: UserRole; label: string; icon: React.ReactNode; desc: string }[] = [
    ...
    { key: 'admin', label: 'Admin Panel', icon: <Crown className="w-3.5 h-3.5 text-[#D9234B]" />, desc: 'Full store manager' },
  ];
  ...
  return (
    <div className="fixed bottom-4 right-4 z-40 hidden sm:block">
      <div className="relative">
        ...
        {isOpen && (
          <div className="absolute right-0 bottom-12 w-64 bg-white text-neutral-900 border border-neutral-200 shadow-2xl p-2 space-y-1 text-xs z-50">
            ...
            {roles.map((r) => (
              <button
                ...
                className={`w-full text-left flex items-start space-x-2.5 p-2 transition-colors ${
                  role === r.key ? 'bg-rose-50 border-l-2 border-[#D9234B]' : 'hover:bg-neutral-100'
                }`}
              >
                <div className="mt-0.5">{r.icon}</div>
                <div>
                  <span className="font-bold text-neutral-900 block">{r.label}</span>
