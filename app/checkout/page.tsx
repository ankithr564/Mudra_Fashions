        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-2.5">
                  {[
                    ...
                    >
                      ...
                      {paymentMethod === pm.key && <Check className="w-4 h-4 text-[#D9234B]" />}
                    </button>
                  ))}
                </div>
          ...
          <div className="p-6 bg-neutral-50 border border-neutral-200 h-fit space-y-4 text-xs shadow-sm">
            ...
            <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
              {cart.map((item) => (
                <div key={item.id} className="flex justify-between border-b border-neutral-200 pb-2">
                  <div>
                    <span className="font-bold text-neutral-900 block truncate max-w-[180px]">{item.product.name}</span>
                    <span className="text-[10px] text-neutral-500">{item.size} / {item.color} x {item.quantity}</span>
                  </div>
                  <span className="font-bold text-neutral-900">₹{(item.unitPrice * item.quantity).toLocaleString('en-IN')}</span>
                </div>
              ))}
            </div>
            ...
            <div className="pt-2 border-t border-neutral-200 space-y-1.5">
              ...
              <div className="flex justify-between font-serif font-bold text-base text-neutral-900 pt-2 border-t border-neutral-200">
